...
void setQueen(int queens[], int row, int col, int id, int col1) { 	//Same as serial just level and parallesim added.
  for(int i=0; i<row; i++) {
    // vertical attacks
    if (queens[i]==col) {
      return;
...
void setQueen1(int queens[], int row, int col, int id, int col1) {
  ...
  if(row==size-1 && col1!=(size/2)) {
    ...
    if(row<=LEVEL){										// New task if row is less then level.
      for(int i=0; i<size; i++) {
        #pragma omp parallel
        #pragma omp single
        {
        ...
        for(int i=0; i<size; i++) {
          setQueen1(queens, row+1, i, id,col1);				// search linear from here.
      }
    }
  }
}

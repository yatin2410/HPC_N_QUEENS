...
void setQueen(int queens[], int row, int col, int id) {				// Same as Serial code just parallel part is added.
  for(int i=0; i<row; i++) {
    // vertical attacks
    if (queens[i]==col) {
      return;
  ...
  if(row==size-1) {
    ...
    for(int i=0; i<size; i++) {
        #pragma omp task						// creating task for each recursion
...
void solve(int p) {
    ...
    #pragma omp parallel
    #pragma omp single				// parallel block
    {
        for(int i=0; i<size; i++) {
